...
int main()
{
    ...
    //-----------------------------------------------------------------------------------
    Shader theShader("upside-down.vs", "shader.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // render
        //-------------------------------------------------------------------------------
        float RED_COLOR = variableColor(2, 0.1);
        float GREEN_COLOR = variableColor(2.1, 0.3);
        float BLUE_COLOR = variableColor(3, 0.4);
        glClearColor(RED_COLOR, GREEN_COLOR, BLUE_COLOR, 1.0f);
        glClear(GL_COLOR_BUFFER_BIT);

        // update the uniform xOffset
...
float variableColor(float period = 1.0f, float phase = 0)
{
    ...
    period *= 5;
    return (2.0f/3.0f) * (sin((2*M_PI / period) * time + phase)/2.0f + 0.5 + sin(1.1f * (2*M_PI / period) * time - phase)/4.0f + 0.25);
}
