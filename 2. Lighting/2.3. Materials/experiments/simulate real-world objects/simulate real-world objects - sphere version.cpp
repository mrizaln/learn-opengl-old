...
namespace orbitProperty
{
    float speed{ 1.0f };
    glm::vec3 orbitAxis{ 0.1f, 0.5f, 0.2f };
}
...
int main()
{
    ...
    //-------------------------
    // a sphere container object
    Object<Sphere> sphere(
        Sphere(0.5f, 64, 32, true),
    ...
    sphere.applyMaterial();

    sphere.getShader().setVec3("light.ambient",  glm::vec3{ 0.2f, 0.2f, 0.2f });
    sphere.getShader().setVec3("light.diffuse",  glm::vec3{ 0.5f, 0.5f, 0.5f });
    sphere.getShader().setVec3("light.specular", glm::vec3{ 1.0f, 1.0f, 1.0f });
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // change material
        sphere.setMaterial(varyingMaterial::material);
        sphere.applyMaterial();

        // set uniform
        sphere.getShader().setVec3("lightPos", light.getPosition());
        sphere.getShader().setVec3("viewPos", camera.position);
        ...
        // model matrix
        sphere.getShader().setMat4("model", glm::rotate(sphere.getModelMatrix(), static_cast<float>(glfwGetTime()), glm::vec3{ 0.0f, 1.0f, 0.0f }));

        // draw
        sphere.getObject().draw();
        ...
        // model is different
        orbit(light.getPosition(), orbitProperty::orbitAxis, sphere.getPosition(), orbitProperty::speed);
...
void orbit(glm::vec3& orbitObjectPosition, const glm::vec3 orbitAxis, const glm::vec3 orbitCenter, float orbitSpeed = 1.0f)
{
    ...
    auto normalizedOrbitAxis{ glm::normalize(orbitAxis) };
    ...
    // transformation axis
    auto transformAxis{ glm::normalize(glm::cross(normalizedOrbitAxis, orbitUp)) };       // this won't work if orbitAxis == orbitUp

    // // align x-axis
    // auto xzAngle{ acos(glm::dot(glm::normalize(glm::vec3{ orbitPosition.x, 0.0f, orbitPosition.z }), glm::vec3{ 1.0f, 0.0f, 0.0f })) };
    // if (orbitPosition.z < 0.0f)
    //     xzAngle += static_cast<float>(M_PI);

    // fromOrbitToWorldCoordinate = glm::rotate(fromOrbitToWorldCoordinate, xzAngle, glm::vec3{ 0.0f, 1.0f, 0.0f });

    // don't do rotation if orbitAxis == orbitUp
    if (normalizedOrbitAxis != orbitUp)
